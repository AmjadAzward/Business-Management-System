...
namespace FinalProject
{
    public partial class Teamleaderp : Form
    {
        ...
        private void LoadTeamLeaderData()
        {
            ...
            try
            {
                ...
                {
                    string query = "SELECT softSkills, educationalQualifications FROM TeamLeader WHERE employeeId = @employeeId";
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                guna2TextBox1.Text = reader["softSkills"].ToString();
                                guna2TextBox2.Text = reader["educationalQualifications"].ToString();
                            }
                            else
                            {
            ...
            {
                MessageBox.Show($"Error loading team leader data: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void guna2CircleButton2_Click(object sender, EventArgs e)
        {
            ...
            Teamleader mainForm = new Teamleader(loggedinuser);
            mainForm.Show();
            this.Hide();
        }

        private void Teamleaderp_Load(object sender, EventArgs e)
        {
            LoadTeamLeaderData();
        }
        ...
        private void guna2Button1_Click(object sender, EventArgs e)
        {
            ...
            string educationalQualifications = guna2TextBox2.Text.Trim();
            string employeeId = idbox.Text.Trim();
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string checkQuery = "SELECT COUNT(1) FROM TeamLeader WHERE employeeId = @employeeId";
                    using (SqlCommand checkCommand = new SqlCommand(checkQuery, connection))
                    {
                        ...
                        if (count > 0)
                        {
                            string updateQuery = "UPDATE TeamLeader " +
                                                 "SET softSkills = @softSkills, educationalQualifications = @educationalQualifications " +
                            ...
                            using (SqlCommand updateCommand = new SqlCommand(updateQuery, connection))
                            {
                                ...
                                MessageBox.Show("Team Leader record updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        else
                        {
                            string insertQuery = "INSERT INTO TeamLeader (employeeId, softSkills, educationalQualifications) " +
                                                 "VALUES (@employeeId, @softSkills, @educationalQualifications)";
                            using (SqlCommand insertCommand = new SqlCommand(insertQuery, connection))
                            {
                                ...
                                MessageBox.Show("Team Leader record added successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                }
