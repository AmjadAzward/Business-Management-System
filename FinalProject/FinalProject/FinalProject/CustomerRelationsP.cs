...
{
    public partial class CustomerRelationsP : Form
    {
        string connectionString = "Server=Ilma_A;Database=finalPJS;Trusted_Connection=True;";

        string loggedinuser = "";

        public CustomerRelationsP(string userName)
        {
            InitializeComponent();
        ...
        private void UpsertCustomerRelation()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@CurrentUserName", loggedinuser); // loggedinuser should contain the current user's username
        ...
        private void guna2CircleButton2_Click(object sender, EventArgs e)
        {
            ...
            CustomerRelations mainForm = new CustomerRelations(loggedinuser);
            mainForm.Show();
            this.Hide();
        }

        private void CustomerRelationsP_Load(object sender, EventArgs e)
        {
            LoadCustomerRelationData();
...

        {
            ...
            // Check if userName, email, or phoneNumber are unique
        ...
        {
            string preferredLanguage = guna2TextBox1.Text.Trim(); // Get Preferred Language
            string softSkills = guna2TextBox2.Text.Trim();        // Get Soft Skills
            string employeeId = idbox.Text.Trim();                 // Get Employee ID
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string checkQuery = "SELECT COUNT(1) FROM CustomerRelation WHERE employeeId = @employeeId";
                    ...
                    if (count > 0)
                    {
                        string updateQuery = "UPDATE CustomerRelation " +
                                             "SET preferredLanguage = @preferredLanguage, softSkills = @softSkills " +
                        ...
                        MessageBox.Show("Customer Relation record updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        string insertQuery = "INSERT INTO CustomerRelation (employeeId, preferredLanguage, softSkills) " +
                                             "VALUES (@employeeId, @preferredLanguage, @softSkills)";
                        ...
                        MessageBox.Show("Customer Relation record added successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            catch (Exception ex)
        ...
        private void LoadCustomerRelationData()
        {
            string employeeId = idbox.Text.Trim();

            if (string.IsNullOrEmpty(employeeId))
            {
                MessageBox.Show("Please provide an Employee ID to load data.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "SELECT preferredLanguage, softSkills FROM CustomerRelation WHERE employeeId = @employeeId";
                    ...
                    if (reader.Read())
                    {
                        ...
                        guna2TextBox2.Text = reader["softSkills"].ToString();
                    }
