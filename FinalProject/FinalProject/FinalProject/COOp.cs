    ...
    public partial class COOp : Form
    {
        ...
        private void LoadCOOData()
        {
            ...
            try
            {
                ...
                {
                    string query = "SELECT communcationChannels, vision FROM COO WHERE employeeId = @employeeId";
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                guna2TextBox1.Text = reader["communcationChannels"].ToString(); 
            ...
            {
                MessageBox.Show($"Error loading COO data: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void COOp_Load(object sender, EventArgs e)
        {
            LoadCOOData();
        }
        ...
        {
            string communcationChannels = guna2TextBox1.Text.Trim(); 
            ...
            if (string.IsNullOrEmpty(communcationChannels))
            {
                MessageBox.Show("Communication Channels cannot be empty.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                guna2TextBox1.Focus();
                return;
            }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string checkQuery = "SELECT COUNT(1) FROM COO WHERE employeeId = @employeeId";
                    using (SqlCommand checkCommand = new SqlCommand(checkQuery, connection))
                    {
                        ...
                        if (count > 0)
                        {
                            string updateQuery = "UPDATE COO " +
                                                 "SET communcationChannels = @communcationChannels, vision = @vision " +
                                                 "WHERE employeeId = @employeeId";
                            using (SqlCommand updateCommand = new SqlCommand(updateQuery, connection))
                            {
                                updateCommand.Parameters.AddWithValue("@communcationChannels", communcationChannels);
                                updateCommand.Parameters.AddWithValue("@vision", vision);
                                updateCommand.Parameters.AddWithValue("@employeeId", employeeId);

                                updateCommand.ExecuteNonQuery();
                                MessageBox.Show("COO record updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        else
                        {
                            string insertQuery = "INSERT INTO COO (employeeId, communcationChannels, vision) " +
                                                 "VALUES (@employeeId, @communcationChannels, @vision)";
                            using (SqlCommand insertCommand = new SqlCommand(insertQuery, connection))
                            {
                                insertCommand.Parameters.AddWithValue("@employeeId", employeeId);
                                insertCommand.Parameters.AddWithValue("@communcationChannels", communcationChannels);
                                insertCommand.Parameters.AddWithValue("@vision", vision);

                                insertCommand.ExecuteNonQuery();
                                MessageBox.Show("COO record added successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                }
