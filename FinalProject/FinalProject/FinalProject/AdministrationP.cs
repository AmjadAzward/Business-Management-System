...
{
    public partial class AdministrationP : Form
    {
        string connectionString = "Server=Ilma_A;Database=finalPJS;Trusted_Connection=True;";

        string loggedinuser = "";
        public AdministrationP(string userName)
        {
            InitializeComponent();
        ...
        private void LoadAdministrationData()
        {
            ...
            try
            {
                ...
                {
                    string query = "SELECT earlyJobTitles, softSkills FROM Administration WHERE employeeId = @employeeId";
        ...
        {
            Administartion mainForm = new Administartion(loggedinuser);
            mainForm.Show();
            this.Hide();
        }
        ...
        private void AdministrationP_Load(object sender, EventArgs e)
        {
        ...
        {
            string earlyJobTitles = guna2TextBox1.Text.Trim(); // Get Early Job Titles
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string checkQuery = "SELECT COUNT(1) FROM Administration WHERE employeeId = @employeeId";
                    ...
                    if (count > 0)
                    {
                        string updateQuery = "UPDATE Administration " +
                                             "SET earlyJobTitles = @earlyJobTitles, softSkills = @softSkills " +
                        ...
                        MessageBox.Show("Administration record updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        string insertQuery = "INSERT INTO Administration (employeeId, earlyJobTitles, softSkills) " +
                                             "VALUES (@employeeId, @earlyJobTitles, @softSkills)";
                        ...
                        MessageBox.Show("Administration record added successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
            }
            catch (Exception ex)
