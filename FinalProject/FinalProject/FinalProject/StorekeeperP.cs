...
namespace FinalProject
{
    public partial class StorekeeperP : Form
    {
        ...
        private void guna2CircleButton2_Click(object sender, EventArgs e)
        {
            ...
            Storekeeper mainForm = new Storekeeper(loggedinuser);
            mainForm.Show();
            this.Hide();
        }
        ...
        private void StorekeeperP_Load(object sender, EventArgs e)
        {
            LoadStorekeeperData();
        }
        ...
        private void LoadStorekeeperData()
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "SELECT experiencedYears, bankDetails FROM Storekeeper WHERE employeeId = @employeeId";
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                guna2TextBox1.Text = reader["experiencedYears"].ToString();
            ...
            {
                MessageBox.Show($"Error loading storekeeper data: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void guna2Button1_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(experiencedYearsText) || !int.TryParse(experiencedYearsText, out int experiencedYears) || experiencedYears <= 0)
            {
                MessageBox.Show("Please enter a valid number of years of experience.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                guna2TextBox1.Focus();
                return;
            }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string checkQuery = "SELECT COUNT(1) FROM Storekeeper WHERE employeeId = @employeeId";
                    using (SqlCommand checkCommand = new SqlCommand(checkQuery, connection))
                    {
                        ...
                        if (count > 0)
                        {
                            string updateQuery = "UPDATE Storekeeper " +
                                                 "SET experiencedYears = @experiencedYears, bankDetails = @bankDetails " +
                            ...
                            using (SqlCommand updateCommand = new SqlCommand(updateQuery, connection))
                            {
                                ...
                                MessageBox.Show("Storekeeper record updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        else
                        {
                            string insertQuery = "INSERT INTO Storekeeper (employeeId, experiencedYears, bankDetails) " +
                                                 "VALUES (@employeeId, @experiencedYears, @bankDetails)";
                            using (SqlCommand insertCommand = new SqlCommand(insertQuery, connection))
                            {
                                ...
                                MessageBox.Show("Storekeeper record added successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                }
