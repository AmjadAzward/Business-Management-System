...
namespace FinalProject
{
    public partial class AAccountantP : Form
    {
        ...
        private void AAccountantP_Load(object sender, EventArgs e)
        {
            LoadAssistantAccountantData();        }
        ...
        private void LoadAssistantAccountantData()
        {
            string employeeId = idbox.Text.Trim();
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "SELECT earlyJobTitles, bankDetails FROM AssistantAccountant WHERE employeeId = @employeeId";
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                guna2TextBox2.Text = reader["bankDetails"].ToString();
                            }
                            else
                            {
            ...
            {
                MessageBox.Show($"Error loading assistant accountant data: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void guna2Button3_Click(object sender, EventArgs e)
        {
            ...
            string bankDetails = guna2TextBox2.Text.Trim();
            string employeeId = idbox.Text.Trim();
            ...
            {
                MessageBox.Show("Bank Details cannot be empty.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                guna2TextBox2.Focus();
                return;
            }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string checkQuery = "SELECT COUNT(1) FROM AssistantAccountant WHERE employeeId = @employeeId";
                    using (SqlCommand checkCommand = new SqlCommand(checkQuery, connection))
                    {
                        ...
                        if (count > 0)
                        {
                            string updateQuery = "UPDATE AssistantAccountant " +
                                                 "SET earlyJobTitles = @earlyJobTitles, bankDetails = @bankDetails " +
                                                 "WHERE employeeId = @employeeId";
                            using (SqlCommand updateCommand = new SqlCommand(updateQuery, connection))
                            {
                                updateCommand.Parameters.AddWithValue("@earlyJobTitles", earlyJobTitles);
                                updateCommand.Parameters.AddWithValue("@bankDetails", bankDetails);
                                updateCommand.Parameters.AddWithValue("@employeeId", employeeId);

                                updateCommand.ExecuteNonQuery();
                                MessageBox.Show("Assistant Accountant record updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        else
                        {
                            string insertQuery = "INSERT INTO AssistantAccountant (employeeId, earlyJobTitles, bankDetails) " +
                                                 "VALUES (@employeeId, @earlyJobTitles, @bankDetails)";
                            using (SqlCommand insertCommand = new SqlCommand(insertQuery, connection))
                            {
                                insertCommand.Parameters.AddWithValue("@employeeId", employeeId);
                                insertCommand.Parameters.AddWithValue("@earlyJobTitles", earlyJobTitles);
                                insertCommand.Parameters.AddWithValue("@bankDetails", bankDetails);

                                insertCommand.ExecuteNonQuery();
                                MessageBox.Show("Assistant Accountant record added successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                }
