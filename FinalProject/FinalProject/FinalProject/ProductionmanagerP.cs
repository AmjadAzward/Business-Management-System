...
{
    public partial class ProductionmanagerP : Form
    {
        string connectionString = "Server=Ilma_A;Database=finalPJS;Trusted_Connection=True;";

        string loggedinuser = "";
        public ProductionmanagerP(string userName)
        {
            InitializeComponent();
        ...
        private void guna2CircleButton2_Click(object sender, EventArgs e)
        {
            ...
            Productionmanager mainForm = new Productionmanager(loggedinuser);
            mainForm.Show();
            this.Hide();
        }
        ...
        private void ProductionmanagerP_Load(object sender, EventArgs e)
        {
            LoadProductionManagerData();
        }
        ...
        private void LoadProductionManagerData()
        {
            string employeeId = idbox.Text.Trim();
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "SELECT earlyJobTitles, vision FROM ProductionManager WHERE employeeId = @employeeId";
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                guna2TextBox1.Text = reader["earlyJobTitles"].ToString();
                                guna2TextBox2.Text = reader["vision"].ToString();
                            }
                            else
                            {
            ...
            {
                MessageBox.Show($"Error loading production manager data: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            string earlyJobTitles = guna2TextBox1.Text.Trim();
            string vision = guna2TextBox2.Text.Trim();
            string employeeId = idbox.Text.Trim();
            ...
            if (string.IsNullOrEmpty(earlyJobTitles))
            {
                MessageBox.Show("Early Job Titles cannot be empty.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                guna2TextBox1.Focus();
                return;
            }
            ...
            {
                MessageBox.Show("Vision cannot be empty.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                guna2TextBox2.Focus();
                return;
            }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string checkQuery = "SELECT COUNT(1) FROM ProductionManager WHERE employeeId = @employeeId";
                    using (SqlCommand checkCommand = new SqlCommand(checkQuery, connection))
                    {
                        ...
                        if (count > 0)
                        {
                            string updateQuery = "UPDATE ProductionManager " +
                                                 "SET earlyJobTitles = @earlyJobTitles, vision = @vision " +
                                                 "WHERE employeeId = @employeeId";
                            using (SqlCommand updateCommand = new SqlCommand(updateQuery, connection))
                            {
                                updateCommand.Parameters.AddWithValue("@earlyJobTitles", earlyJobTitles);
                                updateCommand.Parameters.AddWithValue("@vision", vision);
                                updateCommand.Parameters.AddWithValue("@employeeId", employeeId);

                                updateCommand.ExecuteNonQuery();
                                MessageBox.Show("Production Manager record updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        else
                        {
                            string insertQuery = "INSERT INTO ProductionManager (employeeId, earlyJobTitles, vision) " +
                                                 "VALUES (@employeeId, @earlyJobTitles, @vision)";
                            using (SqlCommand insertCommand = new SqlCommand(insertQuery, connection))
                            {
                                insertCommand.Parameters.AddWithValue("@employeeId", employeeId);
                                insertCommand.Parameters.AddWithValue("@earlyJobTitles", earlyJobTitles);
                                insertCommand.Parameters.AddWithValue("@vision", vision);

                                insertCommand.ExecuteNonQuery();
                                MessageBox.Show("Production Manager record added successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                }
