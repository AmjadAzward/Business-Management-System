...
{
    public partial class TechnicalOfficerP : Form
    {
        string connectionString = "Server=Ilma_A;Database=finalPJS;Trusted_Connection=True;";

        string loggedinuser = "";
        public TechnicalOfficerP(string userName)
        {
            InitializeComponent();
        ...
        private void TechnicalOfficerP_Load(object sender, EventArgs e)
        {
            LoadTechnicalOfficerData();
        }
        ...
        private void LoadTechnicalOfficerData()
        {
            string employeeId = idbox.Text.Trim();
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "SELECT specialization, techSkills FROM TechnicalOfficer WHERE employeeId = @employeeId";
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                guna2TextBox1.Text = reader["specialization"].ToString();
                                guna2TextBox2.Text = reader["techSkills"].ToString();
                            }
                            else
                            {
            ...
            {
                MessageBox.Show($"Error loading technical officer data: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            string specialization = guna2TextBox1.Text.Trim();
            string techSkills = guna2TextBox2.Text.Trim();
            string employeeId = idbox.Text.Trim();
            ...
            {
                MessageBox.Show("Specialization cannot be empty.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                guna2TextBox1.Focus();
                return;
            }

            if (string.IsNullOrEmpty(techSkills))
            {
                MessageBox.Show("Technical Skills cannot be empty.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                guna2TextBox2.Focus();
                return;
            }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string checkQuery = "SELECT COUNT(1) FROM TechnicalOfficer WHERE employeeId = @employeeId";
                    using (SqlCommand checkCommand = new SqlCommand(checkQuery, connection))
                    {
                        ...
                        if (count > 0)
                        {
                            string updateQuery = "UPDATE TechnicalOfficer " +
                                                 "SET specialization = @specialization, techSkills = @techSkills " +
                                                 "WHERE employeeId = @employeeId";
                            using (SqlCommand updateCommand = new SqlCommand(updateQuery, connection))
                            {
                                updateCommand.Parameters.AddWithValue("@specialization", specialization);
                                updateCommand.Parameters.AddWithValue("@techSkills", techSkills);
                                updateCommand.Parameters.AddWithValue("@employeeId", employeeId);

                                updateCommand.ExecuteNonQuery();
                                MessageBox.Show("Technical Officer record updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        else
                        {
                            string insertQuery = "INSERT INTO TechnicalOfficer (employeeId, specialization, techSkills) " +
                                                 "VALUES (@employeeId, @specialization, @techSkills)";
                            using (SqlCommand insertCommand = new SqlCommand(insertQuery, connection))
                            {
                                insertCommand.Parameters.AddWithValue("@employeeId", employeeId);
                                insertCommand.Parameters.AddWithValue("@specialization", specialization);
                                insertCommand.Parameters.AddWithValue("@techSkills", techSkills);

                                insertCommand.ExecuteNonQuery();
                                MessageBox.Show("Technical Officer record added successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                }
