...
namespace FinalProject
{
    public partial class SalesP : Form
    {
        ...
        private void guna2CircleButton2_Click(object sender, EventArgs e)
        {
            ...
            SalesExecutive mainForm = new SalesExecutive(loggedinuser);
            mainForm.Show();
            this.Hide();
        }
        ...
        private void SalesP_Load(object sender, EventArgs e)
        {
            LoadSalesExecutiveData();
        }
        ...
        private void LoadSalesExecutiveData()
        {
            string employeeId = idbox.Text.Trim();
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "SELECT preferredLanguage, experiencedYears FROM SalesExecutive WHERE employeeId = @employeeId";
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                guna2TextBox1.Text = reader["preferredLanguage"].ToString();
                                guna2TextBox2.Text = reader["experiencedYears"].ToString();
                            }
                            else
                            {
            ...
            {
                MessageBox.Show($"Error loading sales executive data: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            string preferredLanguage = guna2TextBox1.Text.Trim();
            string experiencedYears = guna2TextBox2.Text.Trim();
            string employeeId = idbox.Text.Trim();
            ...
            {
                MessageBox.Show("Preferred Language cannot be empty.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                guna2TextBox1.Focus();
                return;
            }

            if (string.IsNullOrEmpty(experiencedYears) || !int.TryParse(experiencedYears, out int years))
            {
                MessageBox.Show("Experienced Years must be a valid number.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                guna2TextBox2.Focus();
                return;
            }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string checkQuery = "SELECT COUNT(1) FROM SalesExecutive WHERE employeeId = @employeeId";
                    using (SqlCommand checkCommand = new SqlCommand(checkQuery, connection))
                    {
                        ...
                        if (count > 0)
                        {
                            string updateQuery = "UPDATE SalesExecutive " +
                                                 "SET preferredLanguage = @preferredLanguage, experiencedYears = @experiencedYears " +
                                                 "WHERE employeeId = @employeeId";
                            using (SqlCommand updateCommand = new SqlCommand(updateQuery, connection))
                            {
                                updateCommand.Parameters.AddWithValue("@preferredLanguage", preferredLanguage);
                                updateCommand.Parameters.AddWithValue("@experiencedYears", years);
                                updateCommand.Parameters.AddWithValue("@employeeId", employeeId);

                                updateCommand.ExecuteNonQuery();
                                MessageBox.Show("Sales Executive record updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        else
                        {
                            string insertQuery = "INSERT INTO SalesExecutive (employeeId, preferredLanguage, experiencedYears) " +
                                                 "VALUES (@employeeId, @preferredLanguage, @experiencedYears)";
                            using (SqlCommand insertCommand = new SqlCommand(insertQuery, connection))
                            {
                                insertCommand.Parameters.AddWithValue("@employeeId", employeeId);
                                insertCommand.Parameters.AddWithValue("@preferredLanguage", preferredLanguage);
                                insertCommand.Parameters.AddWithValue("@experiencedYears", years);

                                insertCommand.ExecuteNonQuery();
                                MessageBox.Show("Sales Executive record added successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                }
