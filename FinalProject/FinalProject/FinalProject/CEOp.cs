...
namespace FinalProject
{
    public partial class CEOp : Form
    {
        ...
        private void FixImageOrientation(ref Image img)
        {
        ...
        private void CEOp_Load(object sender, EventArgs e)
        {
            LoadCEOData();
        }

        private void btnBrowseImage_Click_2(object sender, EventArgs e)
        {
        ...
        private void LoadCEOData()
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "SELECT educationalqualifications, revenueTarget FROM CEO WHERE employeeId = @employeeId";
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                guna2TextBox1.Text = reader["educationalqualifications"].ToString();
                                guna2TextBox2.Text = reader["revenueTarget"].ToString();
                            }
                            else
                            {
            ...
            {
                MessageBox.Show($"Error loading CEO data: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            string educationalqualifications = guna2TextBox1.Text.Trim();
            string revenueTargetText = guna2TextBox2.Text.Trim();
            string employeeId = idbox.Text.Trim();
            ...
            if (string.IsNullOrEmpty(educationalqualifications))
            {
                MessageBox.Show("Educational Qualifications cannot be empty.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                guna2TextBox1.Focus();
                return;
            }

            if (string.IsNullOrEmpty(revenueTargetText) || !decimal.TryParse(revenueTargetText, out decimal revenueTarget))
            {
                MessageBox.Show("Revenue Target must be a valid number.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                guna2TextBox2.Focus();
                return;
            }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string checkQuery = "SELECT COUNT(1) FROM CEO WHERE employeeId = @employeeId";
                    using (SqlCommand checkCommand = new SqlCommand(checkQuery, connection))
                    {
                        ...
                        if (count > 0)
                        {
                            string updateQuery = "UPDATE CEO SET educationalqualifications = @educationalqualifications, revenueTarget = @revenueTarget WHERE employeeId = @employeeId";
                            using (SqlCommand updateCommand = new SqlCommand(updateQuery, connection))
                            {
                                updateCommand.Parameters.AddWithValue("@educationalqualifications", educationalqualifications);
                                updateCommand.Parameters.AddWithValue("@revenueTarget", revenueTarget);
                                updateCommand.Parameters.AddWithValue("@employeeId", employeeId);

                                updateCommand.ExecuteNonQuery();
                                MessageBox.Show("CEO record updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        else
                        {
                            string insertQuery = "INSERT INTO CEO (employeeId, educationalqualifications, revenueTarget) VALUES (@employeeId, @educationalqualifications, @revenueTarget)";
                            using (SqlCommand insertCommand = new SqlCommand(insertQuery, connection))
                            {
                                insertCommand.Parameters.AddWithValue("@employeeId", employeeId);
                                insertCommand.Parameters.AddWithValue("@educationalqualifications", educationalqualifications);
                                insertCommand.Parameters.AddWithValue("@revenueTarget", revenueTarget);

                                insertCommand.ExecuteNonQuery();
                                MessageBox.Show("CEO record added successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                }
