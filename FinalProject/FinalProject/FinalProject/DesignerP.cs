    ...
    public partial class DesignerP : Form
    {
        ...
        private void LoadDesignerData()
        {
            ...
            try
            {
                ...
                {
                    string query = "SELECT earlierProjectLinks, softwareUsed FROM Designer WHERE employeeId = @employeeId";
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                guna2TextBox1.Text = reader["earlierProjectLinks"].ToString();
                                guna2TextBox2.Text = reader["softwareUsed"].ToString();
                            }
                            else
                            {
            ...
            {
                MessageBox.Show($"Error loading designer data: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            Designer mainForm = new Designer(loggedinuser);
            mainForm.Show();
            this.Hide();
        }
        ...
        private void DesignerP_Load(object sender, EventArgs e)
        {
            LoadDesignerData();
        }
        ...
        {
            string earlierProjectLinks = guna2TextBox1.Text.Trim();
            string softwareUsed = guna2TextBox2.Text.Trim();
            string employeeId = idbox.Text.Trim();
            ...
            if (string.IsNullOrEmpty(earlierProjectLinks))
            {
                MessageBox.Show("Earlier project links cannot be empty.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                guna2TextBox1.Focus();
                return;
            }
            ...
            {
                MessageBox.Show("Software used cannot be empty.", "Validation Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                guna2TextBox2.Focus();
                return;
            }
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string checkQuery = "SELECT COUNT(1) FROM Designer WHERE employeeId = @employeeId";
                    using (SqlCommand checkCommand = new SqlCommand(checkQuery, connection))
                    {
                        ...
                        if (count > 0)
                        {
                            string updateQuery = "UPDATE Designer " +
                                                 "SET earlierProjectLinks = @earlierProjectLinks, softwareUsed = @softwareUsed " +
                                                 "WHERE employeeId = @employeeId";
                            using (SqlCommand updateCommand = new SqlCommand(updateQuery, connection))
                            {
                                updateCommand.Parameters.AddWithValue("@earlierProjectLinks", earlierProjectLinks);
                                updateCommand.Parameters.AddWithValue("@softwareUsed", softwareUsed);
                                updateCommand.Parameters.AddWithValue("@employeeId", employeeId);

                                updateCommand.ExecuteNonQuery();
                                MessageBox.Show("Designer record updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        else
                        {
                            string insertQuery = "INSERT INTO Designer (employeeId, earlierProjectLinks, softwareUsed) " +
                                                 "VALUES (@employeeId, @earlierProjectLinks, @softwareUsed)";
                            using (SqlCommand insertCommand = new SqlCommand(insertQuery, connection))
                            {
                                insertCommand.Parameters.AddWithValue("@employeeId", employeeId);
                                insertCommand.Parameters.AddWithValue("@earlierProjectLinks", earlierProjectLinks);
                                insertCommand.Parameters.AddWithValue("@softwareUsed", softwareUsed);

                                insertCommand.ExecuteNonQuery();
                                MessageBox.Show("Designer record added successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                }
