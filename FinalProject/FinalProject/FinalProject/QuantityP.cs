...
namespace FinalProject
{
    public partial class QuantityP : Form
    {
        ...
        private void guna2CircleButton2_Click(object sender, EventArgs e)
        {
            ...
            QuantitySurveyor mainForm = new QuantitySurveyor(loggedinuser);
            mainForm.Show();
            this.Hide();
        }
        ...
        private void QuantityP_Load(object sender, EventArgs e)
        {
            LoadQuantitySurveyorData();
        }
        ...
        private void LoadQuantitySurveyorData()
        {
            string employeeId = idbox.Text.Trim();
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    string query = "SELECT softSkills, bankDetails FROM QuantitySurveyor WHERE employeeId = @employeeId";
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                guna2TextBox1.Text = reader["softSkills"]?.ToString() ?? string.Empty;
                                guna2TextBox2.Text = reader["bankDetails"]?.ToString() ?? string.Empty;
                            }
                            else
                            {
            ...
            {
                MessageBox.Show($"Error loading quantity surveyor data: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        {
            string softSkills = guna2TextBox1.Text.Trim();
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string checkQuery = "SELECT COUNT(1) FROM QuantitySurveyor WHERE employeeId = @employeeId";
                    using (SqlCommand checkCommand = new SqlCommand(checkQuery, connection))
                    {
                        ...
                        if (count > 0)
                        {
                            string updateQuery = "UPDATE QuantitySurveyor " +
                                                 "SET softSkills = @softSkills, bankDetails = @bankDetails " +
                            ...
                            using (SqlCommand updateCommand = new SqlCommand(updateQuery, connection))
                            {
                                ...
                                MessageBox.Show("Quantity Surveyor record updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        else
                        {
                            string insertQuery = "INSERT INTO QuantitySurveyor (employeeId, softSkills, bankDetails) " +
                                                 "VALUES (@employeeId, @softSkills, @bankDetails)";
                            using (SqlCommand insertCommand = new SqlCommand(insertQuery, connection))
                            {
                                ...
                                MessageBox.Show("Quantity Surveyor record added successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                    }
                }
